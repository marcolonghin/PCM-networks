...
closeness = nx.closeness_centrality(base_graph)
ranked_nodes = sorted(closeness, key=closeness.get, reverse=True)
...
plt.xlabel("Removed node (based on closeness centrality)")
plt.ylabel("Similarity")
plt.title("Similarity decay under node removal (by node name)")
plt.legend()
plt.tight_layout()
plt.savefig("similarity_closeness.png")
plt.show()
...
for l1, l2 in pairs:
    ...
    print(
        ...
        .head(20)[["node", "closeness", "delta_similarity"]]
    )
...
# ===============================
# FULL CLOSENESS CENTRALITY TABLE
# ===============================
